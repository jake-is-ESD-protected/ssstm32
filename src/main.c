...
const usb_audio_device_descriptor_config_t usb_audio_device_config = {
  ...
  .bcd_device = 0x0100,
  .manufacturer = "jesdev",
  .product = "STM32 State-Space Audio",
  .serial = "H753SSM001",
  .uac1_function = "STM32 State-Space Audio",
  .uac2_function = "STM32 State-Space Audio",
};

#define USB_AUDIO_SAMPLE_RATE 48000U
#define USB_SPEAKER_CHANNELS  2U  // host -> MCU: stereo S16_LE
#define USB_MIC_CHANNELS      2U  // MCU -> host: stereo S16_LE
...
static volatile int32_t audio_gain_q15[3] = {32767, 32767, 32767};

static float pcm_to_float(int16_t sample) {
...
static int16_t apply_audio_control(float sample, uint8_t channel) {
  if (audio_mute[0] || audio_mute[channel]) return 0;
  return float_to_pcm(sample * (float)audio_gain_q15[channel] / 32767.0f);
}
...
static int16_t mic_ring[MIC_RING_FRAMES][SAMPLES_PER_MS * USB_MIC_CHANNELS];
...
static void update_audio_gain_from_volume_controls(void) {
  for (uint8_t channel = 1; channel < 3U; ++channel) {
    float db = (float)(audio_volume_db256[0] + audio_volume_db256[channel]) / 256.0f;
    float gain = powf(10.0f, db / 20.0f);
    if (gain > 1.995f) gain = 1.995f;
    if (gain < 0.0f) gain = 0.0f;
    audio_gain_q15[channel] = (int32_t)(gain * 32767.0f);
  }
}
